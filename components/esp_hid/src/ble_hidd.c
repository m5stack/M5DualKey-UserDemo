...
static void hid_event_handler(esp_ble_hidd_dev_t *dev, int device_index, esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_DISCONNECT_EVT: {
        ESP_LOGI(TAG, "HID DISCONNECT[%d] 0x%x", device_index, param->disconnect.reason);
        if (dev->connected) {
            dev->connected = false;
...
static esp_err_t esp_ble_hidd_dev_disconnect(void *devp)
{
    esp_ble_hidd_dev_t *dev = (esp_ble_hidd_dev_t *)devp;
    if (!dev || s_dev != dev) {
        return ESP_FAIL;
    ...
    esp_err_t ret = esp_ble_gap_disconnect(dev->remote_bda);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "esp_ble_gap_disconnect failed: %d", ret);
        return ret;
    }
    return ESP_OK;
}
...
static esp_err_t esp_ble_hidd_dev_battery_set(void *devp, uint8_t level)
{
    ...
    ret = esp_ble_gatts_set_attr_value(dev->bat_level_handle, 1, &dev->bat_level);
    if (ret != ESP_OK) {
...
esp_err_t esp_ble_hidd_dev_init(esp_hidd_dev_t *dev_p, const esp_hid_device_config_t *config, esp_event_handler_t callback)
{
    ...
    dev_p->disconnect = esp_ble_hidd_dev_disconnect;
