...
static inline void kbd_handler(keyboard_btn_t *kbd)
{
    ...
    int output_gpio_num = kbd->output_gpio_num ? kbd->output_gpio_num : 1;
    for (int i = 0; i < output_gpio_num; i++) {
...
esp_err_t keyboard_button_create(keyboard_btn_config_t *kbd_cfg, keyboard_btn_handle_t *kbd_handle)
{
    ...
    uint32_t output_gpio_num = kbd_cfg->output_gpio_num ? kbd_cfg->output_gpio_num : 1;

    kbd->button_level = calloc(output_gpio_num * kbd->input_gpio_num, sizeof(uint8_t));
    ESP_GOTO_ON_FALSE(kbd->button_level, ESP_ERR_NO_MEM, exit, TAG, "Failed to allocate memory for button level");

    kbd->debounce_cnt = calloc(output_gpio_num * kbd->input_gpio_num, sizeof(uint8_t));
    ESP_GOTO_ON_FALSE(kbd->debounce_cnt, ESP_ERR_NO_MEM, exit, TAG, "Failed to allocate memory for debounce count");

    kbd->key_data = calloc(output_gpio_num * kbd->input_gpio_num, sizeof(keyboard_btn_data_t));
    ESP_GOTO_ON_FALSE(kbd->key_data, ESP_ERR_NO_MEM, exit, TAG, "Failed to allocate memory for key data");

    kbd->key_release_data = calloc(output_gpio_num * kbd->input_gpio_num, sizeof(keyboard_btn_data_t));
