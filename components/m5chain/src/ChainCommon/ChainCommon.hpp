...
private:
    uart_port_t uartPort = UART_NUM_MAX;  // UART port number
    bool mutexLocked = false;
    TickType_t timeout_ticks = pdMS_TO_TICKS(10000);  // Timeout in FreeRTOS ticks
...
public:
     ...
     *
     * This function sets up the UART communication with the specified parameters,
     * including baud rate, pin assignments, and buffer sizes.
     * It is essential to call this function before using the UART interface.
     *
     * @param uart_num UART port number (UART_NUM_1 or UART_NUM_2).
     * @param baud The baud rate for UART communication (default: 115200).
     * @param txPin The GPIO pin number designated for transmitting data.
     * @param rxPin The GPIO pin number designated for receiving data.
     * @param timeout_ms Timeout duration for reading data in milliseconds (default: 10000ms).
     */
    esp_err_t begin(uart_port_t uart_num, int baud = 115200, int txPin = -1, int rxPin = -1, 
