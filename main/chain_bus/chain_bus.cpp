...
const char* chain_device_type_name(chain_device_type_t type)
{
    switch (type) {
        ...
        case CHAIN_UART_TYPE_CODE:     return "UART";
        case CHAIN_SWITCH_TYPE_CODE:   return "SWITCH";
        case CHAIN_PEDAL_TYPE_CODE:    return "PEDAL";
        case CHAIN_MIC_TYPE_CODE:      return "MIC";
...
void chain_bus_scan_single(int bus_idx)
{
        ...
        if (status->device_status && backup_count > 0) {
            ...
            if (backup_hid_configs) {
                for (uint16_t i = 0; i < backup_count; i++) {
                    memcpy(&backup_hid_configs[i], &status->device_status[i].hid_config, sizeof(chain_device_hid_config_t));
                }
    ...
    for (uint16_t i = 0; i < device_count; i++) {
        ...
        chain_status_t uid_ret = chain->getUID(status->device_status[i].id, UID_TYPE_12_BYTE, 
                                               uid, 12, &operation_status, CHAIN_BUS_DEVICE_TIMEOUT_MS);
        if (uid_ret == CHAIN_OK) {
        ...
        if (!status->device_status[i].hid_config_initialized) {
            ...
            if (status->device_status[i].uid_valid) {
                esp_err_t load_ret = chain_bus_config_load(status->device_status[i].uid, 
                                                           status->device_status[i].type,
        ...
        ESP_LOGD(TAG, "[%s] 设备 %d: ID=%d, 类型=%s (0x%04X)", 
                 bus_names[bus_idx], i+1, 
                 status->device_status[i].id,
                 chain_device_type_name(status->device_status[i].type),
                 status->device_status[i].type);
    }
...
void chain_bus_scan_devices(void)
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        } else if (bus_status[i].last_scan_time == 0 || current_time - bus_status[i].last_scan_time >= CHAIN_BUS_SCAN_INTERVAL_MS) {
            // 定时扫描
            chain_bus_scan_single(i);
            bus_status[i].last_scan_time = current_time;
        }
    ...
    switch (dev_status->type) {
        case CHAIN_KEY_TYPE_CODE: {
            ...
            if (status == CHAIN_OK) {
                if (dev_status->device_data.key_data.button_status != button_status) {
                    ...
                    hid_func_type_t func_type = button_status == 1 ? dev_status->hid_config.key_config.press_down : dev_status->hid_config.key_config.press_release;
        ...
        case CHAIN_JOYSTICK_TYPE_CODE: {
            ...
            if (status == CHAIN_OK) {
                ...
                if (dev_status->device_data.joystick_data.x_value != x_value || 
                    ...
                    if (dev_status->hid_config.joystick_config.xy_move_func != HID_FUNC_NONE) {
                        ...
                        if (dev_status->hid_config.joystick_config.xy_move_func == HID_FUNC_MOUSE_SCROLL
                        ...
                        chain_bus_hid_trigger_function(dev_status->hid_config.joystick_config.xy_move_func, &mouse_report);
                    }
                }
            ...
            if (status == CHAIN_OK && dev_status->device_data.joystick_data.button_status != button_status) {
                ...
                hid_func_params_t params;
                params.mouse_value.buttons = button_status == 1 ? 1 : 0;
                hid_func_type_t func_type = button_status == 1 ? dev_status->hid_config.joystick_config.press_down : dev_status->hid_config.joystick_config.press_release;
                chain_bus_hid_trigger_function(func_type, &params);
            }
        ...
        case CHAIN_ENCODER_TYPE_CODE: {
            ...
            if (status == CHAIN_OK) {
                if (dev_status->device_data.encoder_data.encoder_value != encoder_value) {
                    dev_status->device_data.encoder_data.last_encoder_value = dev_status->device_data.encoder_data.encoder_value;
                    ...
                    if (encoder_delta != 0) {
                        ...
                        if (hid_func == HID_FUNC_MOUSE_PAN) {
                            params.mouse_value.pan = encoder_delta;
                        }
                        else
                        if (hid_func == HID_FUNC_MOUSE_MOVE) {
            ...
            if (status == CHAIN_OK) {
                if (dev_status->device_data.encoder_data.button_status != button_status) {
                    ...
                    hid_func_params_t params;
                    params.mouse_value.buttons = button_status == 1 ? 1 : 0;
                    hid_func_type_t func_type = button_status == 1 ? dev_status->hid_config.encoder_config.press_down : dev_status->hid_config.encoder_config.press_release;
                    chain_bus_hid_trigger_function(func_type, &params);
                    if (status_refresh_queue) {
                        status_refresh_type_t refresh_msg = STATUS_REFRESH_IMMEDIATE;
                        xQueueSend(status_refresh_queue, &refresh_msg, 0);
        ...
        case CHAIN_ANGLE_TYPE_CODE: {
            ...
            if (status == CHAIN_OK) {
                if (dev_status->device_data.angle_data.angle_value != angle_value) {
                    dev_status->device_data.angle_data.last_angle_value = dev_status->device_data.angle_data.angle_value;
                    ...
                    uint16_t last_volume_level = (uint16_t)(dev_status->device_data.angle_data.last_angle_value * 100 / 4095);
                    ...
                    if (volume_level != last_volume_level) {
                        ...
                        if (func_type == HID_FUNC_MOUSE_SCROLL) {
            ...
            switch (dev_status->type) {
                case CHAIN_KEY_TYPE_CODE: {
                    ...
                    while (chain->getKeyButtonPressStatus(dev_status->id, &press_type)) {
                        ...
                        switch (press_type) {
                            case CHAIN_BUTTON_PRESS_SINGLE: press_str = "single click"; break;
                            case CHAIN_BUTTON_PRESS_DOUBLE: press_str = "double click"; break;
                            case CHAIN_BUTTON_PRESS_LONG:   press_str = "long press"; break;
                        }
                        
                        // 记录事件信息
                        snprintf(dev_status->last_event_desc, sizeof(dev_status->last_event_desc), 
                                "key event: %s", press_str);
                        ...
                        switch (press_type) {
                            case CHAIN_BUTTON_PRESS_SINGLE: hid_func = dev_status->hid_config.key_config.single_click; break;
                            case CHAIN_BUTTON_PRESS_DOUBLE: hid_func = dev_status->hid_config.key_config.double_click; break;
                            case CHAIN_BUTTON_PRESS_LONG:   hid_func = dev_status->hid_config.key_config.long_press; break;
                        }
                        hid_func_params_t params;
                        ...
                        ESP_LOGI(TAG, "[%s] KEY设备 %d 事件: %s", 
                                 bus_names[bus_idx], dev_status->id, press_str);
                    }
                    break;
                }
                ...
                case CHAIN_PIR_TYPE_CODE: {
                    ...
                    while (chain->getPIRDetectTrigger(dev_status->id, &pir_trigger)) {
                        ...
                        switch (pir_trigger) {
                            case CHAIN_PIR_REPORT_NO_PERSON: detect_str = "no person"; break;
                            case CHAIN_PIR_REPORT_PERSON:    detect_str = "person"; break;
                        }
                        ...
                        ESP_LOGI(TAG, "[%s] PIR设备 %d 事件: %s", 
                                 bus_names[bus_idx], dev_status->id, detect_str);
                    }
                    break;
                }
                ...
                case CHAIN_JOYSTICK_TYPE_CODE: {
                    ...
                    while (chain->getJoystickButtonPressStatus(dev_status->id, &joy_press_type)) {
                        ...
                        hid_func_type_t hid_func = HID_FUNC_NONE;
                        switch (joy_press_type) {
                            case CHAIN_BUTTON_PRESS_SINGLE: hid_func = dev_status->hid_config.joystick_config.single_click; break;
                            case CHAIN_BUTTON_PRESS_DOUBLE: hid_func = dev_status->hid_config.joystick_config.double_click; break;
                            case CHAIN_BUTTON_PRESS_LONG:   hid_func = dev_status->hid_config.joystick_config.long_press; break;
                        }
                        hid_func_params_t params;
                        params.mouse_value.buttons = 1;
                        chain_bus_hid_trigger_function(hid_func, &params);
                        ...
                        ESP_LOGI(TAG, "[%s] JOYSTICK设备 %d 按键事件: %s", 
                                 bus_names[bus_idx], dev_status->id, press_str);
                    }
                    break;
                }
                ...
                case CHAIN_SWITCH_TYPE_CODE: {
                    ...
                    while (chain->getSwitchTrigger(dev_status->id, &switch_trigger)) {
                        ...
                        switch (switch_trigger) {
                            case CHAIN_SWITCH_REPORT_CLOSE: slip_str = "closed"; break;
                            case CHAIN_SWITCH_REPORT_OPEN:  slip_str = "opened"; break;
                        }
                        
                        // 记录事件信息
                        snprintf(dev_status->last_event_desc, sizeof(dev_status->last_event_desc), 
                                "switch status: %s", slip_str);
                        dev_status->last_event_time = esp_timer_get_time() / 1000;
                        dev_status->event_updated = true;
                        dev_status->status_updated = true;
                        ...
                        ESP_LOGI(TAG, "[%s] SWITCH设备 %d 事件: %s", 
                                 bus_names[bus_idx], dev_status->id, slip_str);
                    }
                    break;
                }
                ...
                case CHAIN_ENCODER_TYPE_CODE: {
                    ...
                    while (chain->getEncoderButtonPressStatus(dev_status->id, &press_type)) {
                        ...
                        hid_func_type_t hid_func = HID_FUNC_NONE;
                        switch (press_type) {
                            case CHAIN_BUTTON_PRESS_SINGLE: hid_func = dev_status->hid_config.encoder_config.single_click; break;
                            case CHAIN_BUTTON_PRESS_DOUBLE: hid_func = dev_status->hid_config.encoder_config.double_click; break;
                            case CHAIN_BUTTON_PRESS_LONG:   hid_func = dev_status->hid_config.encoder_config.long_press; break;
                        }
                        hid_func_params_t params;
                        params.mouse_value.buttons = 1;
                        chain_bus_hid_trigger_function(hid_func, &params);
                        ...
                        ESP_LOGI(TAG, "[%s] ENCODER设备 %d 按键事件: %s", 
                                 bus_names[bus_idx], dev_status->id, press_str);
                    }
                    break;
                }
                ...
                case CHAIN_PEDAL_TYPE_CODE: {
                    // 踏板设备事件检查
                    ESP_LOGD(TAG, "[%s] PEDAL设备 %d: 暂不支持事件检查", 
                             bus_names[bus_idx], dev_status->id);
                    break;
                }
                ...
                case CHAIN_MIC_TYPE_CODE: {
                    // 麦克风设备事件检查
                    ESP_LOGD(TAG, "[%s] MIC设备 %d: 暂不支持事件检查", 
                             bus_names[bus_idx], dev_status->id);
                    break;
                }
...
esp_err_t chain_bus_find_device_index(int bus_idx, uint8_t device_id, int* device_index)
{
    ...
    for (int i = 0; i < bus_status[bus_idx].device_count; i++) {
        if (bus_status[bus_idx].device_status[i].id == device_id && 
            bus_status[bus_idx].device_status[i].connected) {
            *device_index = i;
            return ESP_OK;
        }
    }
...
void chain_bus_init_device_hid_config(chain_device_status_t* device_status, int bus_idx)
{
    ...
    switch (device_status->type) {
        ...
        case CHAIN_JOYSTICK_TYPE_CODE:
            ...
            device_status->hid_config.joystick_config.xy_move_reverse = bus_idx == 0 ? true : false; // 左总线默认翻转，右总线默认不翻转
...
esp_err_t chain_bus_set_device_hid_config(int bus_idx, uint8_t device_id, const chain_device_hid_config_t* config)
{
    ...
    switch (status->device_status[device_index].type) {
        case CHAIN_KEY_TYPE_CODE:
            memcpy(&status->device_status[device_index].hid_config.key_config, &config->key_config, sizeof(chain_key_hid_config_t));
            break;
        case CHAIN_JOYSTICK_TYPE_CODE:
            memcpy(&status->device_status[device_index].hid_config.joystick_config, &config->joystick_config, sizeof(chain_joystick_hid_config_t));
            break;
        case CHAIN_ENCODER_TYPE_CODE:
            memcpy(&status->device_status[device_index].hid_config.encoder_config, &config->encoder_config, sizeof(chain_encoder_hid_config_t));
            break;
        case CHAIN_ANGLE_TYPE_CODE:
            memcpy(&status->device_status[device_index].hid_config.angle_config, &config->angle_config, sizeof(chain_angle_hid_config_t));
            break;
        default:
