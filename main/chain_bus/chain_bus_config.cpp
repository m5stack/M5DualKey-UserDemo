...
void chain_bus_uid_to_string(const uint8_t* uid, char* str_buf)
{
    ...
    sprintf(str_buf, "%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X%02X",
...
static void generate_nvs_key(const uint8_t* uid, char* key_buf)
{
    ...
    sprintf(key_buf, "c_%02X%02X%02X%02X%02X%02X",
            uid[0], uid[1], uid[2], uid[3], uid[4], uid[5]);
}
...
static uint32_t calculate_config_crc(const chain_device_saved_config_t* config)
{
    return esp_crc32_le(0, (const uint8_t*)config, 
                        sizeof(chain_device_saved_config_t) - sizeof(uint32_t));
}
                                 ...
                                 chain_device_hid_config_t* hid_config)
{
    ...
    if (saved_config.device_type != device_type) {
        char uid_str[25];
        chain_bus_uid_to_string(uid, uid_str);
        ESP_LOGW(TAG, "设备类型不匹配 [UID:%s] 保存:%d 当前:%d", 
                 uid_str, saved_config.device_type, device_type);
        return ESP_ERR_INVALID_ARG;
    }
