...
void tinyusb_cdc_send_keyboard_report(keyboard_btn_report_t report)
{
    ...
    // 格式化键盘报告为可读字符串
    len += snprintf(buffer + len, sizeof(buffer) - len, "Keys pressed: %ld, released: %ld, changed: %d\r\n",
    ...
    if (report.key_pressed_num > 0) {
        ...
        for (int i = 0; i < report.key_pressed_num; i++) {
            len += snprintf(buffer + len, sizeof(buffer) - len, "(%d,%d) ", report.key_data[i].output_index,
    ...
    if (report.key_release_num > 0) {
        ...
        for (int i = 0; i < report.key_release_num; i++) {
            len += snprintf(buffer + len, sizeof(buffer) - len, "(%d,%d) ", report.key_release_data[i].output_index,
