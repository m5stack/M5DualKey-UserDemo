...
esp_err_t tinyusb_hid_init(void)
{
    ...
    set_string_desc_product(USB_HID_PRODUCT);
...
{
    if (!s_tinyusb_hid) {
        ESP_LOGE(TAG, "TinyUSB HID not initialized");
        return;
    }

    if (xQueueSend(s_tinyusb_hid->hid_queue, &report, pdMS_TO_TICKS(100)) != pdTRUE) {
        ESP_LOGW(TAG, "Mouse report queue is full");
    }
}
